      ...
      (error, user) => {
        if (error) {
          ...
          if (user.length > 0) {
            ...
            const query = `insert into orders (userId, userName,address,city,province,codePostal,total)
            values
            (${userId},'${userName}','${address}','${city}','${province}','${codePostal}',${total});select LAST_INSERT_ID()`;
        ...
        (error, user) => {
          if (error) {
            ...
            if (user.length > 0) {
              let userId = user[0].id;
              pool.query(
                `select orderId, DATE_FORMAT(orderDate,'%m/%d/%Y') as orderDate,userName, address,city,province,codePostal,total from orders where userId = ${userId}`,
                (error, orders) => {
                  if (error) {
                    res.status(500).send({
                      error: error.code,
